...
function bindAuth(){
  ...
  document.getElementById('registerForm').onsubmit=async e=>{e.preventDefault();
    const payload={email:document.getElementById('regEmail').value,password:document.getElementById('regPassword').value,role:document.getElementById('regRole').value};
...
function bindCommon(){
  document.getElementById('exportPdfBtn').onclick=()=>downloadFile('/api/StudentAchievements/export-pdf','achievements-report.pdf');
  document.getElementById('exportExcelBtn').onclick=()=>downloadFile('/api/StudentAchievements/export-excel','achievements-report.xlsx');
  document.getElementById('openPassportBtn').onclick=()=>downloadFile('/api/Passport/export-pdf','model-passport.pdf');
...
async function bootstrapData(){ loadDropdowns(); await Promise.all([loadProfile(),loadPassport(),loadAchievements(),...moduleConfigs.map(loadModule)]); }

async function authFetch(url,opts={}){ return fetch(`${API_URL}${url}`,{...opts,headers:{...(opts.headers||{}),Authorization:`Bearer ${currentToken}`}}); }

async function loadProfile(){ const r=await authFetch('/api/StudentAchievements/profile'); if(!r.ok)return; const p=await r.json(); currentProfile=p; document.getElementById('profileInfo').innerHTML=`<p><strong>ФИО:</strong> ${p.fullName}</p><p><strong>Должность:</strong> ${p.position}</p><p><strong>Место работы:</strong> ${p.workplace}</p><p><strong>Email:</strong> ${p.email}</p>`; document.getElementById('userName').textContent='👋 '+p.fullName; renderPassportPreview(); }
...
async function loadAchievements(){ const r=await authFetch('/api/StudentAchievements/my-achievements'); if(!r.ok)return; const data=await r.json(); document.getElementById('achievementsList').innerHTML=data.map(a=>`<div class="achievement-item"><div class="achievement-info"><strong>${a.studentName}</strong> — ${a.achievementType}<p>${a.level} | ${a.eventDate}</p></div><div class="achievement-actions"><button class="btn-edit" onclick="editAchievement(${a.id})">✏️</button><button class="btn-delete" onclick="deleteAchievement(${a.id})">🗑️</button></div></div>`).join('')||'<p>Нет данных</p>'; }
document.getElementById('achievementForm').onsubmit=async e=>{e.preventDefault(); const payload={studentName:document.getElementById('studentName').value,achievementType:document.getElementById('achievementType').value,eventDate:document.getElementById('eventDate').value,eventOrganizer:document.getElementById('eventOrganizer').value,groupName:document.getElementById('groupName').value,resultDescription:document.getElementById('resultDescription').value,academicyearId:+document.getElementById('academicyearId').value,levelId:+document.getElementById('levelId').value,directionId:null,resultId:null}; const r=await authFetch('/api/StudentAchievements/achievements',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); if(r.ok){document.getElementById('achievementForm').reset();loadAchievements();loadPassport();}};
...
function showDashboard(){showPage('dashboardPage'); document.getElementById('userInfo').style.display='flex';}
function loadDropdowns(){ document.getElementById('levelId').innerHTML='<option value="1">Школьный</option><option value="2">Городской</option>'; document.getElementById('academicyearId').innerHTML='<option value="1">2024-2025</option><option value="2">2025-2026</option>';}
