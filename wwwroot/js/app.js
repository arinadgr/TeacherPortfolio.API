...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof apiClient === 'undefined') return alert('Не загружен api.js. Нажмите Ctrl+F5');

  bindAuth();

  try { bindTabs(); } catch (e) { console.error('bindTabs error', e); }
  try { renderModuleSections(); } catch (e) { console.error('renderModuleSections error', e); }
  try { if (apiClient.getToken()) showDashboardAndLoad(); } catch (e) { console.error('bootstrap error', e); }
});
...
function bindTabs() {
  ...
  const passportBtn = document.getElementById('openPassportBtn'); if (passportBtn) passportBtn.onclick = () => apiClient.download('/api/Passport/export-pdf', 'model-passport.pdf');
}
